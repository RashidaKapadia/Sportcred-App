...
public class DBPosts {
    ...
    public static String createPost(String username, String content, String title){
        ...
        try(Session session = Connect.driver.session()){
            ...
            parameters("z", id, "x", username,"y", content, "u", title,"b", new HashSet<Object>(), "c", new HashSet<Object>(),"d", new ArrayList<String>(), "e", timestamp.toString())));// COMMENTS INITIALIAZATION NEEDS TO BE UPDATED
            session.close();
            return id;
        }
        catch(Exception e){
